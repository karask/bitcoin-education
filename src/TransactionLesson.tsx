...
function example(network: Network): TransactionDraft {
  return {
    inputs: [{ txid: '0123456789abcdef'.repeat(4), vout: '0', amount: '100000', sourceType: 'address', source: addresses[network][0] }],
...
export function TransactionLesson({ runtime }: { runtime: ReturnType<typeof usePython> }) {
  ...
  function build(next = draft, net = network) {
    setSelected('tx-0'); setDirty(false);
    calculate({ kind: 'transaction', transaction: next, network: net, publicKey: '', compressed: true });
  }
  ...
  return <div className="transaction-lesson">
    <section className="hero"><div className="hero-copy"><div className="hero-meta"><span className="chapter-tag">TRANSACTIONS / 01</span><span>P2PKH · UNSIGNED</span></div><h1>A transaction,<br /><span>piece by piece.</span></h1><p>Choose the coins. Create the outputs. Follow every byte.</p></div><div className="tx-hero-diagram" aria-hidden="true"><span>UTXOs</span><ArrowRight size={24} /><span>New outputs</span><small>the difference becomes the fee</small></div></section>
    ...
    <form className="tx-builder" onSubmit={(event) => { event.preventDefault(); build(); }}>
      ...
      <p className="tx-context">The example uses a fictional UTXO. Enter your own details below; existence, ownership, and unspent status are not checked. No private keys are needed.</p>
    ...
    {data && result && active && <>
      ...
      <p className="tx-context">The fee depends on the previous amounts you supplied. It has no field in the transaction. This unsigned draft is {result.byteLength} bytes; signatures will increase its size, so this is not a final fee-rate estimate.</p>
      <section className="panel tx-inspection" aria-label="Transaction hex explorer"><div className="tx-card-heading"><div><span className="output-kicker">02 / FOLLOW THE BYTES</span><h2>The unsigned transaction</h2></div><CopyValue value={result.hex} label="Copy transaction hex" /></div><p className="tx-context">Click, hover, or focus a colored field. Empty scriptSigs occupy zero bytes and appear only in the field list. This draft cannot spend these P2PKH outputs until signed.</p>
        <div className="hex-output tx-hex" data-testid="transaction-hex" aria-label="Unsigned transaction hexadecimal">{data.fields.filter((field) => field.end > field.start).map((field) => <button type="button" key={field.id} className={`byte-field tx-color-${field.category} ${active.id === field.id ? 'inspected' : ''}`} onClick={() => setSelected(field.id)} onMouseEnter={() => setSelected(field.id)} onFocus={() => setSelected(field.id)} aria-label={field.label} aria-pressed={active.id === field.id} aria-describedby="tx-field-description">{result.hex.slice(field.start * 2, field.end * 2).match(/.{2}/g)?.map((byte, i) => <span className="hex-byte" key={i}>{byte}</span>)}</button>)}</div>
      ...
      <details className="panel tx-python"><summary>Inspect the previous locking scripts</summary><p>These describe the UTXOs being spent. They are not inserted into the empty scriptSigs.</p>{data.previousScripts.map((script, i) => <div key={i}><h4>Input {i + 1} · previous scriptPubKey</h4><code className="tx-selected-hex">{script}</code></div>)}</details>
