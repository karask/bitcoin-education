  ...
  return <>
    ...
    <aside id="lesson-navigation" className={`sidebar ${open ? 'open' : ''}`} aria-label="Learning navigation" onKeyDown={(event) => {
      ...
      <nav>
        ...
        <div className="subnav">{LESSON_ORDER.map((id) => <a key={id} href={`#${id}`} onClick={close} aria-current={kind === id ? 'page' : undefined}><span className="active-dot" /><span>{CATALOG[id].nav}</span><span className="nav-tag">{id === 'nested' ? 'WRAPPED' : CATALOG[id].tag}</span></a>)}</div>
      </nav>
